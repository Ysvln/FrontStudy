...
- 예제
  ...
  - javascript ES6부터는 import/export 구문으로 모듈을 다룰 수 있게 되면서 각각의 모듈은 `import/export` 구문에 의해 의존성을 추적 가능
...
- chunk.js나 main.js 같은 이름들의 번들 파일들을 확인
- index.html 파일도 확인
- 빌드 시 index.html에 만들어진 번들 파일들이 적용되어 있는 것
- 빌드를 완료 했으면, 리액트를 실행
...
- react 실행 시 작동 순서
  1. react-scripts로 실행 시 파일들을 처음 빌드하여 번들 파일로 만든다.
  2. 번들 파일들이 반영된 index.html을 만든다
  3. 웹 브라우저 실행 후 접속을 한다.(localhost:3000)
  4. 웹팩이 준비한 index.html에서 scripts 태그의 자바스트립트 코드를 실행하여 웹 페이지에 보여준다.
  5. 웹팩 서버는 새로운 파일의 생성 수정 삭제를 감시한다.
  6. 처음 빌드 한 번들 파일과 비교 후 변경된 부분이 있다면, 해당 부분만 빌드한다.
  7. 브라우저의 웹팩과 상호작용하여 변경 된 부분을 실시간으로 반영한다.
...
[참고 링크3 - 컴파일과 번들링 개념. SASS 파일 컴파일 하면서 Gulp 사용한 경험이 생각나서](https://han-py.tistory.com/448)  
